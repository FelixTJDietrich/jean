...
fn build_wsl_claude_script(
    ...
    if !env_vars.is_empty() || !env_clear.is_empty() {
        inner.push_str("env ");
        // `-u KEY` actually *unsets* an inherited var; a plain `KEY=` assignment
        // would only blank it, which Claude still treats as configured. Unsets
        // come first so ambient Anthropic auth can't override the account's own
        // credentials set below.
        for key in env_clear {
            inner.push_str(&format!("-u {} ", wsl_shell_quote(key)));
        }
        inner.push_str(
            &env_vars
                .iter()
...
mod tests {
    ...
    fn test_wsl_claude_script_detaches_with_setsid() {
        ...
        // Ambient auth vars are unset (`-u`), not blanked, and come before ours
        // are set — a trailing `KEY=` assignment would still read as configured.
        assert!(script.contains(r"| env -u '\''ANTHROPIC_API_KEY'\'' JEAN_MCP_TOKEN="));
