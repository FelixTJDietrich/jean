...
/// Defensive cap for active sessions hydrated during `/api/init`.
/// Stale browser/UI state can otherwise make mobile Safari load too much work
/// during startup.
const MAX_INIT_ACTIVE_SESSIONS: usize = 8;
...
async fn init_handler(
    ...
    // Mobile web asks to skip active-session payloads: their windowed message
    // history is the heaviest part of /api/init, and the client loads the one
    // session it actually opens on demand.
    ...
    let (worktrees_by_project, sessions_by_worktree, mut active_sessions): (
        ...
        let active_future = async {
            if skip_active_sessions {
                ...
                load_active_sessions_windowed(state.app.clone(), &worktrees, &active_ids).await
            }
        };
    ...
    if !cleaned_active_sessions.is_empty() {
        ...
        let fallback_futures: Vec<_> = cleaned_active_sessions
            ...
            .filter_map(|(worktree_id, fallback_id)| {
                // ui_state cleanup below still runs; only the payload is skipped.
                if skip_active_sessions {
                    return None;
                }
