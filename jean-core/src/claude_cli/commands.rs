...
fn load_claude_credentials(
    ...
    // over a potentially stale ~/.claude/.credentials.json. It is skipped entirely
    // when a named account is active — the keychain has no notion of profiles, so
    // reading it would hand back the wrong account's token.
    #[cfg(target_os = "macos")]
    {
...
fn load_stale_cached_claude_usage(cache_key: &str, now_secs: u64) -> Option<ClaudeUsageSnapshot> {
    let path = get_claude_usage_cache_path(cache_key)?;
    let content = std::fs::read_to_string(path).ok()?;
    let entry: ClaudeUsageCacheEntry = serde_json::from_str(&content).ok()?;
...
fn claude_credentials_or_env_authenticated(app: &AppHandle) -> bool {
    // An ambient API key only counts when no named account is active: Jean
    // deliberately scrubs `ANTHROPIC_API_KEY` from account-scoped Claude
    // children (see `spawn_claude_command`), so it says nothing about whether
    // the selected account can actually authenticate.
    if accounts::get_active_account_config_dir(app).is_none() && claude_env_api_key_present() {
        return true;
    }
    match load_claude_credentials(app) {
...
pub async fn check_claude_cli_auth(app: AppHandle) -> Result<ClaudeAuthStatus, String> {
    ...
    if output.status.success() {
        ...
        if claude_credentials_or_env_authenticated(&app) {
            log::trace!(
...
pub(crate) async fn get_claude_usage_with_source(
    ...
    if response.status() == reqwest::StatusCode::TOO_MANY_REQUESTS {
        if let Some(stale) = load_stale_cached_claude_usage(&cache_key, now_secs) {
            log::info!(
